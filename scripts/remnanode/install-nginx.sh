...
update_nginx_config() {
    ...
    cp "/opt/Noderepkot/data/nginx/nginx-node.conf" /etc/nginx/nginx.conf

    rm -f /etc/nginx/conf.d/default.conf
    rm -f /etc/nginx/sites-enabled/default

    if [[ "$USE_PROXY_PROTOCOL" == "y" || "$USE_PROXY_PROTOCOL" == "Y" ]]; then
        cp "/opt/Noderepkot/data/nginx/selfsteal-proxy-protocol.conf" /etc/nginx/conf.d/selfsteal.conf
    else
        cp "/opt/Noderepkot/data/nginx/selfsteal.conf" /etc/nginx/conf.d/selfsteal.conf
    fi
