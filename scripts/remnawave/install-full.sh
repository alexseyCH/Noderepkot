...
install_components() {
    if [ "$REINSTALL_PANEL" = true ]; then
        ...
        cp "/opt/Noderepkot/data/docker/panel.env" .env
        cp "/opt/Noderepkot/data/docker/panel-compose.yml" docker-compose.yml
    ...
    if [ "$REINSTALL_CADDY" = true ]; then
        ...
        cp "/opt/Noderepkot/data/caddy/caddyfile" Caddyfile
        cp "/opt/Noderepkot/data/docker/caddy-compose.yml" docker-compose.yml
